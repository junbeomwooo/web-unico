...
/** 2) 환경설정 정보 */
// 설정파일의 절대 경로 문자열 생성
// resolve() --> 현재 vsc 워크스페이스 의경로
const configFileName = process.env.NODE_ENV !== "production" ? ".env.server.development" : ".env.server.production"; // 상용버전이 아니라면 .env.server.development를 사용하여라
const configPath = join(resolve(), configFileName); // 워크스페이스의 현재 경로 + 파일 이름

// 파일이 존재하지 않을 경우 강제로 에러 발생.
